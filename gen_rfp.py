...
rfp_template = "./templates/RFP Technical Responses.docx"

class RFP(Command):
    def __init__(self):
        super().__init__(
            command_keyword="rfp",
            help_message="Generate RFP Technical Docs!",
            chained_commands=[RfpCallback()])
        

    def execute(self, message, attachment_actions, activity):

        text1 = TextBlock("Generate RFP Tech Docs", weight=FontWeight.BOLDER, size=FontSize.MEDIUM, horizontalAlignment="Center")
        ...
        cube = Toggle(title="Cube", id="cube")
        analog_vg = Toggle(title="Analog VG", id="analog_vg")
        ...
        webex_meetings_text = TextBlock("If Webex Meetings", horizontalAlignment="Center")
        webex_users = Text(id="webex_users", placeholder="Enter the Number of Webex users")
        cube_text = TextBlock("If Cube", horizontalAlignment="Center")
        srst_devices = Text(id="srst_devices", placeholder="Enter the Number of SRST Devices")
        ...
        cube_column = Column(items=[cube], width=2)
        analog_vg_column = Column(items=[analog_vg], width=2)
        ...
        webex_users_column = Column(items=[webex_users], width=2)
        srst_devices_column = Column(items=[srst_devices], width=2)
        ...
        card = AdaptiveCard(
            body=[ColumnSet(columns=[Column(items=[text1], width=2)]),
                  ...
                  ColumnSet(columns=[webex_meetings_column, cube_column, analog_vg_column]),
                  ...
                  ColumnSet(columns=[Column(items=[webex_meetings_text], width=2)]),
                  ColumnSet(columns=[webex_users_column]),
                  ColumnSet(columns=[Column(items=[cube_text], width=2)]),
                  ColumnSet(columns=[srst_devices_column]),
                  ], actions=[submit])

        return response_from_adaptive_card(card)

class RfpCallback(Command):

    def __init__(self):
        super().__init__(
            card_callback_keyword="rfp_callback",
    ...
    def execute(self, message, attachment_actions, activity):
        ...
        for k, v in data.items():
            ...
            try:
                ...
                pass
        generate_doc(rfp_template,"rfp", data)
        res = Response()
